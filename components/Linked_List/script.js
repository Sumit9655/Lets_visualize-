...
const update = (oldData, newData, parentValue, childValue) => {
    ...
    const nodes = d3
        ...
        .attr('class', (d) => (d.data.value != null ? (d.data.nodeId == 0 ? 'topelement' : 'node') : 'null-node'));
    nodes
        .append('circle')
...
const addNode = async () => {
    ...
    while (true) {
        ...
        if (node.value == val) {
            ...
            alert('Value already exists in queue');
...
const deleteNodeRecur = (newData, val) => {
    ...
    if (val != newData.value) {
        alert('Only first element is allowed to delete');
        return newData;
    } else {
        let secondnode = newData.children[0];
        secondnode.nodeId=0;
        return secondnode;
    }
};
...
const deleteNode = async () => {
    ...
    while (true) {
        if (node.value == null) {
            alert('Value is not present in queue');
            update(oldData, newData, -1, -1);
            unfreezeButtons();
            break;
        }
